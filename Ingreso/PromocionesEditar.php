...
<div class="container">
	...
	<form method="post" class="col-xl-8 offset-2">
		...
		<select name="pro_id" class="col-xl-8 offset-2" >
			...
			<option type="text" <?php if($produ["pro_id"] == $data['Detalle'][0]['pro_id'] ) echo "selected";?> value="<?=$produ["pro_id"]?>"><?= $produ["pro_nombre"];?></option>
			<?php endforeach?>
		</select>	
		<!-- Hasta aca-->
        <!-- Desde aca para el foreach-->
		<select name="tpromo_id" class="col-xl-8 offset-2" >
			...
			<?php foreach($tipo["Detalle"] as $tipo):?>	
			<option type="text" <?php if($tipo["tpromo_id"] == $data['Detalle'][0]['tpromo_id'] ) echo "selected";?> value="<?=$tipo["tpromo_id"]?>"><?= $tipo["tpromo_nombre"];?></option>
			<?php endforeach?>
		</select>	
		<!-- Hasta aca-->

		<input type="text" name="promo_descuento" placeholder="Promo Descuento" value="<?= $data["Detalle"][0]["promo_descuento"];?>" class="form-control">
		<input type="input" name="promo_fechaini" value="<?= $data["Detalle"][0]["promo_fechaini"];?>"  class="form-control">

		<input type="input" name="promo_fechafin" placeholder="Fecha Final" class="form-control" value="<?= $data["Detalle"][0]["promo_fechaini"];?>">
