            ...
            <div id="layoutSidenav_content">
                <main>
                        ...
                        <h1>Lista de Stock</h1>
                            <a href="StRegistrar.php" class="btn btn-primary">Registrar stock</a>
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><?=$cliente["st_CantidadAdquirida"]?></td>
                                    <td><?=$cliente["st_CantidadDisponible"]?></td>
                                    <td><?=$cliente["pro_nombre"]?></td>
                                    <td><?=$cliente["pro_descripcion"]?></td>
                                    <td><?=$cliente["pro_PrecioUnitario"]?></td>
                                    <td><?=$cliente["pro_marca"]?></td>
                                    <td><a href="StEditar.php?id=<?= $cliente['st_id']?>&usuid=<?= $_GET['usuid']?>&emp=<?= $_GET['emp']?>&sucu=<?=$_GET['sucu']?>" class="btn btn-warning">Editar</a></td>
                                    <td><a href="StEliminar.php?id=<?= $cliente['st_id']?>&usuid=<?= $_GET['usuid']?>&emp=<?= $_GET['emp']?>&sucu=<?=$_GET['sucu']?>" class="btn btn-danger">Eliminar</a></td>
                                </tr>
                                <?php endforeach?>
                            </tbody>
                        </table>
